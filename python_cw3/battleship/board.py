...
class Board:
    ...
    def are_ships_within_bounds(self): 
        ...
        """
        # Check that every cell of all the ships is within bounds.
        within_bounds = True
        for ship in self.ships:
            for cell in ship.cells:
                if(cell[0] < 1 or cell[0] > self.width):
                    within_bounds = False
                if(cell[1] < 1 or cell[1] > self.height):
                    within_bounds = False
    ...
    def are_ships_too_close(self):
        ...
        Returns:
            bool : return True if and only if there is at least a pair of 
                ...
                otherwise
        """
        # For each ship loop through the other ships and check if they're too close.
        too_close = False
        for ship in self.ships:
            for other_ship in self.ships:
                if((other_ship.x_start, other_ship.y_start) != (ship.x_start, ship.y_start) or (other_ship.x_end, other_ship.y_end) != (ship.x_end, ship.y_end)):
                    if(not too_close):
                        too_close = ship.is_near_ship(other_ship)
        return too_close
    ...
    def have_all_ships_sunk(self):
        ...
        Returns:
            bool : return True if all ships on the board have sunk.
               return False otherwise.
        """
        # Loop through ships checking if they have sunk.
        all_sunk = True
        for ship in self.ships:
            if(all_sunk):
                all_sunk = ship.has_sunk()
    ...
    def is_attacked_at(self, cell):
        ...
        # ship is hit if ship occupies cell.
        is_ship_hit = False
        has_ship_sunk = False
        for ship in self.ships:
            is_ship_hit = ship.is_occupying_cell(cell)
            # ship recieves damage if it is hit.
            if(is_ship_hit):
...
if __name__ == '__main__':
    ...
    # Automatic board
    board = Board(ships_per_length={1: 1, 2: 1, 3: 4, 4: 1, 5: 3},size=(20,10))
    print(board.ships)
    board.print(show_ships=True)
